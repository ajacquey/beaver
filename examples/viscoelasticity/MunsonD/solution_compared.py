...
def read_analytical_data():
    filename = "MunsonDawson_analytical.csv"
    t, gamma_vp = np.loadtxt(filename, delimiter=',', skiprows=1, usecols=[0, 4], unpack=True)
    return t, gamma_vp

def read_simulation_data():
    filename = "MunsonD_csv.csv"  # Replace with your actual filename
    t, gamma_vp = np.loadtxt(filename, delimiter=',', skiprows=1, usecols=[0, 3], unpack=True)
    return t, gamma_vp

def plot_figure():
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.set_title(r'$\gamma_{vp}$ vs Time (days)')  # Fixed LaTeX syntax
    ...
    ax.plot(t_analytical, gamma_vp_exp, color="k", label="MunsonDawson_analytical _ $\gamma_{vp}$", linewidth=2)
    # Plot simulation data
    ax.plot(t_sim, gamma_vp_sim, color="r", label="Munson_Dawson_simulation _ $\gamma_{vp}$", linewidth=2)

    # Set x-axis limit
    ax.set_xlim(None, None)  # Automatically adjust x-axis limits
