...
def read_analytical_data():
    filename = "Lemaitre_analytical.csv"
    t, gamma_vp = np.loadtxt(filename, delimiter=',', skiprows=1, usecols=[0, 1], unpack=True)
    return t, gamma_vp
...
def plot_figure():
    ...
    ax.plot(t_analytical, gamma_vp_exp, color="k", label="Lemaitre_analytical _ $\gamma_{vp}$", linewidth=2)
    # Plot simulation data
    ax.plot(t_sim, gamma_vp_sim, color="r", label="Lemaitre_simulation _ $\gamma_{vp}$", linewidth=2)
