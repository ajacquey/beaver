...
[Functions]
  [pressure_fct]
    type = ParsedFunction
    expression = 'if(t<=0,0.0,1.0-erf(sqrt(x^2+y^2)/sqrt(t)))' 
  []
[]
...
[Outputs]
  ...
  [out]
    type = Exodus
    file_base = 'outputs/constant_friction_injection_3D_T_${T}'
  []
  [max]
    type = CSV
    file_base = 'outputs/constant_friction_injection_3D_T_${T}'
  ...
  [fault]
    type = CSV
    file_base = 'outputs/constant_friction_injection_3D_T_${T}/constant_friction_injection_3D_T_${T}'
    time_data = true
  []
[]
